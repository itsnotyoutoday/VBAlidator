...
CATEGORY_KEYWORDS: dict[str, tuple[str, ...]] = {
    ...
    "block_terminator": ("without", "expected end"),
    "param_list": ("paramarray", "parameter", "optional", "byref"),
    "loop_label": ("exit for", "exit do", "next control", "already in use", "for each", "duplicate label"),
    "placement_def_option": ("invalid inside", "deftype", "duplicate option", "duplicate definition"),
    "conditional_compilation": ("#if", "#else", "#end if", "#const"),
    "lexer_limits": ("unterminated", "missing end bracket", "exceeding", "characters long"),
    "proc_consistency": ("ambiguous name", "inconsistent", "does not match the interface"),
    "declared_type": ("not defined", "no members"),
    "assignment_semantics": ("assignment to", "me keyword", "new keyword"),
    ...
    "statement_placement": ("module level", "procedure body", "executable", "only legal"),
}
